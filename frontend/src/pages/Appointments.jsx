 ...
 const daysOfWeek = ['SUN','MUN','TUE','WED','THU','FRI','SAT'];
  ...
  const getAvailableSlot =async() =>{
     ...
     for(let i=0;i<7;i++){
         ...
         //setting and time of the data with index
    ...
    {docSlots.length  && docSlots.map((item, index) => (
      ...
      <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
        {docSlots.length && docSlots[slotIndex].map((item,index)=>(
            <p onClick={()=>setSlotTime(item.time)} className={`tex-sm font-lightflex-shrink-0 px-5 py-5 py-2 rounded-full cursor-pointer ${item.time === slotTime?'bg-primary text-white':'text-gray-400 border border-gray-300' }`} key={index}>
